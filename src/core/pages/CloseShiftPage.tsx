...
const CloseShiftPage = () => {
    ...
    const fetchSummary = async () => {
      ...
      if (shiftId === 'active') {
        ...
        try {
          ...
          const activeShift = shiftsResponse.data.results.find(shift => shift.is_active);
          if (!activeShift) {
  ...
  const totalDifference = calculateDifference(getTotalExpected(), getTotalActual());
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm">
                    Магазин {summary.store} • Открыта {new Date(summary.opened_at).toLocaleString('ru-RU')}
                  </p>
                </div>
              </div>
            </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 rounded-lg">
                      <th className="text-left py-4 px-6 font-semibold text-gray-700 rounded-l-lg">Тип</th>
                      <th className="text-right py-4 px-4 font-semibold text-gray-700">Поступило</th>
                      <th className="text-right py-4 px-4 font-semibold text-gray-700">Ушло</th>
                      <th className="text-right py-4 px-4 font-semibold text-gray-700">Ожидается</th>
                      <th className="text-right py-4 px-4 font-semibold text-gray-700">Фактически</th>
                      <th className="text-right py-4 px-6 font-semibold text-gray-700 rounded-r-lg">Разница</th>
                    </tr>
                  </thead>
                  <tbody className="space-y-2">
                    {summary.payments.map((payment, index) => {
                      const actualValue = payments[index]?.actual || 0;
                      const difference = calculateDifference(payment.expected, actualValue);
                      
                      return (
                        <tr key={payment.payment_method} className="hover:bg-blue-50 transition-colors duration-200 border-b border-gray-100">
                          ...
                          <td className="py-4 px-6 text-right">
                            ...
                            >
                              {difference.isZero ? '0' : (difference.isPositive ? '+' : '-') + difference.amount.toLocaleString()}
                            </span>
                          </td>
                        </tr>
                      );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
              <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-4">
                <h3 className="text-lg font-bold text-white">💰 Наличные в кассе</h3>
              </div>
              <div className="p-6">
            ...
            <div className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="bg-blue-50 rounded-xl p-4 border-l-4 border-blue-500">
                  <div className="text-sm text-blue-600 font-medium">Всего продаж</div>
                  <div className="text-2xl font-bold text-blue-800">{summary.total_sales_count}</div>
                </div>
                <div className="bg-green-50 rounded-xl p-4 border-l-4 border-green-500">
                  <div className="text-sm text-green-600 font-medium">Сумма продаж</div>
                  <div className="text-2xl font-bold text-green-800">{summary.total_sales_amount.toLocaleString()} UZS</div>
                </div>
                <div className="bg-orange-50 rounded-xl p-4 border-l-4 border-orange-500">
                  <div className="text-sm text-orange-600 font-medium">Возвратов</div>
                  <div className="text-2xl font-bold text-orange-800">{summary.total_returns_count}</div>
                </div>
                <div className="bg-red-50 rounded-xl p-4 border-l-4 border-red-500">
                  <div className="text-sm text-red-600 font-medium">Сумма возвратов</div>
                  <div className="text-2xl font-bold text-red-800">{summary.total_returns_amount.toLocaleString()} UZS</div>
                </div>
              </div>
              
              <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-6 space-y-4">
                <div className="flex justify-between items-center py-3 border-b border-gray-200">
                  <span className="text-lg font-semibold text-gray-700">Ожидается:</span>
                  <span className="text-2xl font-bold text-purple-600">{getTotalExpected().toLocaleString()} UZS</span>
                </div>
                <div className="flex justify-between items-center py-3 border-b border-gray-200">
                  <span className="text-lg font-semibold text-gray-700">Фактически:</span>
                  <span className="text-2xl font-bold text-blue-600">{getTotalActual().toLocaleString()} UZS</span>
                </div>
                <div className="flex justify-between items-center py-4 bg-white rounded-xl px-6 shadow-md">
                  <span className="text-xl font-bold text-gray-800">Разница:</span>
                  <span
                  ...
                  >
                    {totalDifference.isZero ? '0' : (totalDifference.isPositive ? '+' : '-') + totalDifference.amount.toLocaleString()} UZS
                  </span>
                </div>
              </div>
            </div>
