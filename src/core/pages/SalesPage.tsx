    ...
    sale.sale_items?.forEach((item: any, index: number) => {
      ...
      if (product?.id && stock?.id) {
        ...
        if (recyclingRecord) {
          ...
          const profit = calculateRecyclingProfit(recyclingRecord, Number(item.quantity), Number(item.subtotal));
      ...
      if (!recyclingProfitUsed && product?.category_read?.category_name === 'Рейка') {
        console.log('Calculation Type: Standard Product (Рейка without recycling record)');
