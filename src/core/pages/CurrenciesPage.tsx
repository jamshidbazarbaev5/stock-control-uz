...
import { type Currency, useGetCurrencies, useUpdateCurrency, useDeleteCurrency } from '../api/currency';
...
const columns = (t: any) => [
  ...
  },
  {
    ...
    accessorKey: 'is_base',
    cell: ({ row }: any) => {
      return row?.original?.is_base ? t('common.yes') : t('common.no');
    },
  },
];
...
export default function CurrenciesPage() {
  ...
  const currencies = (currenciesData ) as any || [];

  // Enhance currencies with display ID
  const enhancedCurrencies = currencies.map((currency: Currency, index: number) => ({
    ...currency,
    displayId: index + 1
  }));
  ...
  const handleUpdateSubmit = (e: React.FormEvent) => {
    ...
    updateCurrency(updateData, {
      onSuccess: () => {
        toast.success(t('messages.success.updated', { item: t('navigation.currencies') }));
        setIsFormOpen(false);
      ...
      onError: () => toast.error(t('messages.error.update', { item: t('navigation.currencies') })),
    });
  };

  const handleDelete = (id: number) => {
    deleteCurrency(id, {
      onSuccess: () => toast.success(t('messages.success.deleted', { item: t('navigation.currencies') })),
      onError: () => toast.error(t('messages.error.delete', { item: t('navigation.currencies') })),
    });
  };
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-md">
          <div className="space-y-6">
            <h2 className="text-xl font-semibold">{t('messages.edit')} {t('navigation.currencies')}</h2>
            
            <form onSubmit={handleUpdateSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="edit_currency_name">{t('forms.currency_name')} *</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="edit_short_name">{t('forms.short_name')} *</Label>
                <Input
                  id="edit_short_name"
                  type="text"
                  value={editShortName}
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="edit_is_base"
                  checked={editIsBase}
                  onCheckedChange={(checked) => setEditIsBase(checked as boolean)}
                />
                <Label htmlFor="edit_is_base" className="text-sm font-normal cursor-pointer">
                  {t('forms.is_base')}
                </Label>
              </div>
