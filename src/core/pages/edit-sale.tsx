...
//   useEffect(() => {
//     if (sale && sale.store_read?.id) {
//       const storeId = sale.store_read.id;
...
//       const payments = sale.sale_payments?.map(payment => ({
...
//           client: sale.sale_debt.client,
//           due_date: sale.sale_debt.due_date,
//         } : undefined
//       });

//       // Set selected stocks and prices
//       const stocksData: Record<number, number> = {};
//       const pricesData: Record<number, { min: number; selling: number }> = {};

//       sale.sale_items.forEach((item, index) => {
//         if (item.stock_read) {
//           stocksData[index] = item.stock_read.quantity || 0;
//           pricesData[index] = {
//             min: parseFloat(item.stock_read.min_price || '0'),
//             selling: parseFloat(item.stock_read.selling_price || '0')
//           };
//         }
//       });

//       setSelectedStocks(stocksData);
//       setSelectedPrices(pricesData);
//     }
//   }, [sale, form]);
...
//   const addSaleItem = () => {
//     const items = form.getValues('sale_items');
//     form.setValue('sale_items', [...items, { stock_write: 0, selling_method: 'Штук', quantity: 1, subtotal: '0' }]);
//     updateTotalAmount();
//   };
...
//         {t('common.edit')} {t('navigation.sale')} #{id}
//       </h1>
...
//                   <SelectContent>
//                     {stores.map((store) => (
//                       <SelectItem key={store.id} value={store.id?.toString() || ''} disabled={!store.id}>
//                         {store.name}
//                       </SelectItem>
//                     ))}
//                   </SelectContent>
